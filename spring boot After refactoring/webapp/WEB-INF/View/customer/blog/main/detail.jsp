	...
	<div id="detail">
		...
		<div id="thumbnail">
			<img id="thumbnail" style="width: 100%; height: auto;"
				src="/upload/image/fileupload/${board.num}/thumbnail/thumbnail.PNG"
			...
			<tag:forEach var="x" items="${comment}">
				<tag:if test="${x.ref_comment == 0}">
					<li id="comment_area${x.num}">
						<p>
							<b id="user-name${ x.num }">${ x.user_name.replaceAll("_BLOG_USER","") }</b> <span> <fmt:formatDate
						...
						<p id="edit${x.num}">
							<tag:if test="${ x.user_name !='NULL' }">
								...
								<fn onclick="DelComment(${x.board_num }, ${x.num})">삭제</fn>
							</tag:if>
						</p>
					</li>
					...
					<tag:forEach var="y" items="${recomment}" varStatus="status">
						<tag:if test="${ y.ref_comment == x.num }">
							<li id="comment_area${y.num}" class="recomment"><i
								...
								<p>
									<b id="user-name${y.num}">${y.user_name.replaceAll("_BLOG_USER","") }</b> <span><fmt:formatDate
								...
								<p id="edit${y.num}">
									<tag:if test="${ y.user_name !='NULL' }">
										...
										<fn onclick="DelComment(${y.board_num }, ${y.num})">삭제</fn>
									</tag:if>
								</p></li>
