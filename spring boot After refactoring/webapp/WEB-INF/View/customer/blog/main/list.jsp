...
<div class="Main-Content">
		...
		<tag:forEach var="x" items="${list}">
			<tag:if test="${!x.hidden.equals('Y')}">
				<li><a href="/detail/${x.num}"><img
						...
						onerror="this.src='/image/logo/black.png'"  alt="${x.title} 썸네일"/> <span>
							<h3>${x.title }</h3>
							<p>${x.category}
								º
	...
	<ul class="page_controller">
		...
		<tag:forEach var="i" begin="${StartNum}" end="${StartNum + 4}">
			<tag:if test="${i <= LastNum }">
				...
				href="${param.category}?title=${param.title}&page=${i}">
						${i}</a></li>
